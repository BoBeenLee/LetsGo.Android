 ...
 * which is then parsed and converting to a com.example.googlemap.app.route overlay using some classes created by Hesham Saeed.
 * @author Joel Dean
...
public class Routing extends AsyncTask<LatLng, Void, List<Route>> {
    ...
    protected void dispatchOnSuccess(List<Route> result) {
        for (RoutingListener mListener : _aListeners) {
            mListener.onRoutingSuccess(result);
        }
    }
     ...
     */
    @Override
    protected List<Route> doInBackground(LatLng... aPoints) {
    ...
    protected void onPostExecute(List<Route> result) {
        if (result == null) {
            ...
            for(int i=0; i<result.size(); i++){
                Log.v("test", result.get(i).getDistance());
            }
